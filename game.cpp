...
void Game::recv_handle() {
	...
	*/
	...
	/*
		超过1分钟没有反应，
	*/
	if (pClient1 != NULL && (pClient1->m_lastRecvTime + 60000 <= timeGetTime())) {
		log("[WARNING::DISCONNECT_CLIENT1]%s no response over 1 min IN recv_handle\r\n\r\n\r\n", pClient1->m_IP);
		DisconnectClient(pClient1);
	}
	else if (pClient2 != NULL && pClient2->m_lastRecvTime + 60000 <= timeGetTime()) {
		log("[WARNING::DISCONNECT_CLIENT2]%s no response over 1 min IN recv_handle\r\n\r\n\r\n", pClient2->m_IP);
		DisconnectClient(pClient2);
	}
